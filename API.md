...
```
SetTime(u, controller, datetime, timeout)

where:
- u             Uhppoted        Uhppoted struct initialised with the bind address, broadcast address, etc
- controller    controller      uint32|Controller controller serial number or {id, address, protocol} Controller struct
- datetime      datetime        date/time to which to set controller system time
- timeout       time.Duration   maximum time to wait for a response from a controller
```
...
Creates a scheduled task.
```
AddTaskRecord(u, controller, task, timeout)

where:
- u             Uhppoted        Uhppoted struct initialised with the bind address, broadcast address, etc
- controller    controller      uint32|Controller controller serial number or {id, address, protocol} Controller struct
- task          Task            Task struct intialised with the scheduled task details.
- timeout       time.Duration   maximum time to wait for a response from a controller
```
...
```
SetFirstCard(u, controller, door, firstcard, timeout)

where:
- u             Uhppoted        Uhppoted struct initialised with the bind address, broadcast address, etc
- controller    controller      uint32|Controller controller serial number or {id, address, protocol} Controller struct
- door          door            door ID [1..4]
- firstcard     FirstCard       FirstCard struct intialised with the first-card configuration.
- timeout       time.Duration   maximum time to wait for a response from a controller
```
...
type FirstCard struct {
  StartTime     HHmm          `json:"start-time"`      // time from which the first-card configuration is active
  EndTime       HHmm          `json:"end-time"`        // time after which the first-card configuration is no longer active
  ActiveMode    DoorMode      `json:"active-mode"`     // active door mode (controlled, normally open or normally closed)
  InactiveMode  DoorMode      `json:"inactive-mode"`   // inactive door mode (controlled, normally open, normally closed or firstcard only)
  Weekdays      Weekdays      `json:"weekdays"`        // days of week on which profile is enabled
}
